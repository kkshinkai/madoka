...
impl<T, E> HasThat<T> for Result<T, E> where T: PartialEq {
    ...
    fn has_any(&self, xs: &[T]) -> bool where T: PartialEq {
        xs.iter().any(|x| self.has_a(x))
    }
...
mod tests {
    ...
    fn test_option_contains() {
        ...
        assert!(Some(1).has_any(&[1, 2, 3]));
        assert!(!Some(4).has_any(&[1, 2, 3]));
        assert!(!None.has_any(&[1, 2, 3]));
    ...
    fn test_result_contains() {
        ...
        assert!(Ok::<_, ()>(1).has_any(&[1, 2, 3]));
        assert!(!Ok::<_, ()>(4).has_any(&[1, 2, 3]));
        assert!(!Err(()).has_any(&[1, 2, 3]));
